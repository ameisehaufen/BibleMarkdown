...
bibles_dir="$PWD"
biblia=`printQuestion -m ver-*.txt`
echo "Biblia=$biblia"
extensao=txt
...
Echo "Dica: Lembre-se, se quiser imagens descompacte o arquivo Images.zip para a pasta corrente, gerando uma pasta Images."
...
1)
	[ ! -f $biblia ] && exit 1
	mkdir "${livros_abrev_pref[0]}" && touch "${livros_abrev_pref[0]}"/0.$extensao
	contador_livros=1
	...
	for i in "${livros[@]}"; do
	    ...
	    for j in "${capitulos[@]}"; do
	        ...
	        echo -e "# ${livros_nomes[$contador_livros]} Cap ${nr_cap}\n" >> "${livros_abrev_pref[$contador_livros]}"/${nr_cap}.$extensao
	        count=1
	        for k in "${versiculos[@]}"; do
	            ...
	            for l in "${bible_pictures[@]}"; do
	                if [ -f "Images/SweetPublishing/$l" ]; then
	                    echo -en "![](../Images/SweetPublishing/$l) " >> "${livros_abrev_pref[$contador_livros]}"/${nr_cap}.$extensao
	                fi
	            done
...
2)
	...
	if printQuestion -y "Corrigir links dos versiculos no csv?"; then
		sed -i "s/<a class='bible' \(href='\)#b\([A-Za-z0-9\-]\+\).\([0-9]*\).\([0-9]*\)\(\.*[0-9-]*\)*'/<a \1..\/\2\/\3.${extensao}#\4' /g" working.csv
		sed -i "s/class='bible'//g" working.csv
		sed -i "s/\/\([0-9]\)\.$extensao/\/0\1\.$extensao/g" working.csv
	fi

	if printQuestion -y "Inserir comentários nos arquivos .$extensao?"; then
		...
		for i in "${comentarios[@]}"; do
			...
			for j in "${com_capitulo[@]}"; do
				...
				if [ "$versiculo" -ge 0 ]; then
					...
					if [ -z "$(cat "${livros_abrev_pref[$livro]}"/$capitulo.$extensao | grep "\*\*"$versiculo"\*\*")" ]; then
						if [ ! $(cat "${livros_abrev_pref[$livro]}"/$capitulo.$extensao | grep "\*\*Cmt $nickname\*\* Intro:") ]; then
							echo -en "\n> **Cmt $nickname** Intro: $j" >> "${livros_abrev_pref[$livro]}"/$capitulo.$extensao
						else
							echo -en "> $j" >> "${livros_abrev_pref[$livro]}"/$capitulo.$extensao
						fi
					else
						sed -i "/\*\*$versiculo\*\*/a linha_em_branco\n> **Cmt $nickname**: $j" "${livros_abrev_pref[$livro]}"/$capitulo.$extensao
					fi
				else
					if [ ${#j} -gt 10 ]; then
						if [ ! $(cat "${livros_abrev_pref[$livro]}"/$capitulo.$extensao | grep "\*\*Cmt $nickname\*\* Intro:") ]; then
...
7)
	...
	1)
		for i in "${livros_abrev_pref[@]}"; do
			...
			cd "$i"
			for j in *.$extensao; do
				pandoc --latex-engine=xelatex -V urlcolor=cyan "$j" -o "$(basename $j .$extensao)".pdf
	...
	2)
		for i in "${livros_abrev_pref[@]}"; do
			# cria um pdf para cada capitulo
			cd "$i"
			pandoc --latex-engine=xelatex -V urlcolor=cyan *.$extensao -o ../"$i".pdf
			# junta todos os capitulos em um doc pdf apenas... tentar Colocar um header com todos os livro bem pequenos, em cada livro os seus resp capitulos, se estiver no cap, o versiculo de cada livro.
			cd -
		done
		;;
